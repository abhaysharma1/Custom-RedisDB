...
static void out_str(Buffer& out, const char* s, size_t size) {
    buf_append_u8(out, TAG_STR);               // write tag
    buf_append_u32(out, (uint32_t)size);       // write size
    buf_append(out, (const uint8_t*)s, size);  // write data
}
...
int main() {
    ...
    setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &val,
               sizeof(val));  // allows resuing of the same ports when server restart without
                              // TCP time wait
    ...
    while (true)  // infinte server listening
    {
        ...
        // a pollarg.event is a bitmap for telling the kernel to check whether the fd is ready
        // for us to perform certain task or not the pollarg.revents tells us if it is ready to
        // perform the tasks we specified
        ...
        // and tell if we can perform them using revents(bitmap of the operations the fd is
        // ready for)
